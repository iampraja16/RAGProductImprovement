...
- analyze_smr: Gunakan untuk menganalisis HM (Hour Meter) atau SMR (Service Meter Reading) / jam operasi unit. Contoh: "hydraulic leak muncul di SMR/HM berapa saja?", "pada jam berapa engine overheat terjadi?", "distribusi SMR untuk masalah oli bocor", "smr/hm analysis final drive leak", "oil leak di site Jembayan lengkap dengan SMR nya", "tampilkan scatter plot HM untuk final drive leak di Samarinda", "masalah hydraulic leak di site Jembayan dilengkapi data HM". Tool ini mengembalikan data HM/SMR numerik untuk divisualisasikan dalam scatter plot. JANGAN gunakan ask_emr_database untuk query HM/SMR — gunakan tool ini.

Aturan Kritis:
- Jika user bertanya tentang jumlah, tren, ranking, agregasi data, perbandingan frekuensi, ATAU "komponen mana yang paling sering rusak", SELALU gunakan ask_emr_database. TAPI JIKA PERTANYAAN MENGANDUNG HM/SMR, abaikan aturan ini dan gunakan analyze_smr.
...
- Jika user menyebut site/lokasi TERTENTU (nama site seperti Jembayan, Samarinda, dll.) BERSAMA masalah spesifik (seperti hydraulic leak, engine overheat) DAN meminta data HM/SMR/jam/scatter plot, SELALU gunakan analyze_smr — karena tool ini support filter site + masalah sekaligus dan mengembalikan scatter plot.
